...
def draw():
    ...
    pgm.add_node(daft.Node("ytrue", r"$y_n$", 1.3, 1.4, fixed=True, offset=(6,-19)))

    # Data - observed y:
    pgm.add_node(daft.Node("y", r"$D_n$", 2.5, 1.4, observed=True))

    # Constants - x and errors:
    pgm.add_node(daft.Node("sigma", r"$\sigma_n$", 1.9, 0.9, fixed=True, offset=(-3,2)))
    pgm.add_node(daft.Node("x", r"$x_n$", 0.8, 1.1, fixed=True))
    ...
    pgm.add_plate(daft.Plate('datapoints', [0.5, 0.7, 2.7, 1.4], label=r"N datapoints", shift=-0.1))
